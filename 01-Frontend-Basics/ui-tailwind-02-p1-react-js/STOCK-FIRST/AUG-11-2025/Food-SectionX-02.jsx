    ...
    <div className="w-full bg-blue-100 py-12 px-4 sm:px-6 lg:px-16 font-sans">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-14 gap-x-8 justify-items-center">
        {foods.map((food, index) => (
          <div
            key={index}
            className="w-full max-w-xs bg-white border border-gray-100 shadow-xl rounded-2xl p-6 pt-12 relative flex flex-col items-center transition hover:shadow-2xl"
          >
            {/* Image */}
            <div className="absolute -top-14">
              <div className="w-28 h-28 rounded-full overflow-hidden border-4 border-green-400 bg-white shadow-md">
                <img
                  src={food.image}
                  alt={food.title}
            ...
            {/* Content */}
            <h3 className="mt-16 text-lg font-semibold text-gray-800 text-center">
              {food.title}
            </h3>
