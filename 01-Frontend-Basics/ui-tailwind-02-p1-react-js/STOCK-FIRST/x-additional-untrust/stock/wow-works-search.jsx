...
export default function Home() {
  ...
  const handleChange = (e) => {
    ...
    // Autocomplete suggestions (partial match)
    if (value) {
  ...
  // Filter quotes based on partial keyword match
  const filteredQuotes = search
    ...
    : quotesArray; // Show all quotes if search is empty
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center pt-20 px-4">
      ...
      <div className="mt-6 w-full max-w-md space-y-4">
        ...
        {filteredQuotes.map((quote, index) => (
          <div
            key={index}
            className="bg-white p-4 rounded-lg shadow hover:shadow-md transition border-2 border-blue-400"
          >
