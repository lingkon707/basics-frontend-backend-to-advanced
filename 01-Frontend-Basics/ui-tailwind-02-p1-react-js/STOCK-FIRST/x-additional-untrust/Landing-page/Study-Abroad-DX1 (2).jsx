      ...
      <div className="max-w-7xl mx-auto text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-8">
          {tools.map((tool, index) => (
            <div
              key={index}
              className="relative bg-white p-6 rounded-xl shadow-lg hover:shadow-2xl transition-shadow duration-500 transform hover:scale-105 hover:translate-y-2 hover:ease-in-out"
            >
              <img
                src={tool.image}
                alt={tool.title}
                className="w-full h-48 object-cover rounded-full border-4 border-indigo-200 mb-4 transition-all duration-500 ease-in-out transform hover:scale-110"
              />
              <h3 className="text-2xl font-semibold text-gray-800 mb-2 transition-opacity duration-500 opacity-0 hover:opacity-100">{tool.title}</h3>
              <p className="text-gray-600 mb-4 transition-opacity duration-700 opacity-0 hover:opacity-100 delay-200">{tool.description}</p>
              <button className="text-green-500 hover:text-green-700 font-medium border-b-2 border-transparent hover:border-green-500 transition-all duration-500 ease-in-out">
                Explore tool
              </button>
            </div>
          ))}
        ...
        <div className="mt-12 space-x-4 text-center">
          <button className="px-8 py-3 bg-green-600 text-white rounded-full font-semibold hover:bg-green-700 transition duration-500 ease-in-out">
            See how we partner
          </button>
          <button className="px-8 py-3 bg-transparent text-green-600 border-2 border-green-600 rounded-full font-semibold hover:bg-green-100 transition duration-500 ease-in-out">
            Explore all tools
          </button>
        </div>
      </div>
