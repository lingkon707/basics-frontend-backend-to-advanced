      ...
      <div className="max-w-7xl mx-auto text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-8">
          {tools.map((tool, index) => (
            <div
              key={index}
              className="relative bg-white p-6 rounded-xl shadow-lg hover:shadow-2xl transition-shadow duration-300 transform hover:scale-105"
            >
              <img
                src={tool.image}
                alt={tool.title}
                className="w-full h-48 object-cover rounded-full border-4 border-indigo-200 mb-4 transition-all duration-300"
              />
